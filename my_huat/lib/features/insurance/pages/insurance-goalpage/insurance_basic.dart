// features/insurance/goals/insurance_basic.dart
...
class InsuranceBasicPage extends StatefulWidget {
  const InsuranceBasicPage({super.key});

  @override
  State<InsuranceBasicPage> createState() => _InsuranceBasicPageState();
}

class _InsuranceBasicPageState extends State<InsuranceBasicPage> {
  // Dropdown selection
  String? _selectedTopic;
  final List<String> _topics = [
    'Essence of Insurance Selection',
    'Types of Insurance Company in Malaysia'
  ];

  // Video players
  late VideoPlayerController _comprehensiveController;
  late VideoPlayerController _companyController;
  late Future<void> _comprehensiveInitializeFuture;
  late Future<void> _companyInitializeFuture;
  ...
  void initState() {
    ...
    // Initialize comprehensive video controller (Essence of Insurance Selection)
    _comprehensiveController = VideoPlayerController.asset('assets/video/Comprehensive.mp4');
    _comprehensiveInitializeFuture = _comprehensiveController.initialize().then((_) {
      _comprehensiveController.setLooping(false);
      _comprehensiveController.setVolume(1.0);
      if (mounted) setState(() {});
    }).catchError((error) {
      debugPrint('Error loading comprehensive video: $error');
    });

    // Initialize company video controller (Types of Insurance Company in Malaysia)
    _companyController = VideoPlayerController.asset('assets/video/company.mp4');
    _companyInitializeFuture = _companyController.initialize().then((_) {
      _companyController.setLooping(false);
      _companyController.setVolume(1.0);
      if (mounted) setState(() {});
    }).catchError((error) {
      debugPrint('Error loading company video: $error');
    });
  }
  ...
  void _onTopicChanged(String? newValue) {
    setState(() {
      ...
      if (newValue == 'Essence of Insurance Selection') {
        _activeController = _comprehensiveController;
        _comprehensiveInitializeFuture.then((_) {
          if (mounted) {
            setState(() {
      ...
      } else if (newValue == 'Types of Insurance Company in Malaysia') {
        _activeController = _companyController;
        _companyInitializeFuture.then((_) {
          if (mounted) {
            setState(() {
              _isVideoInitialized = true;
            });
  ...
  List<Map<String, dynamic>> _getQuestionsForTopic() {
    if (_selectedTopic == 'Essence of Insurance Selection') {
      return [
        {
          'question': 'Which of the following is one of the three major types of insurance mentioned?',
          'options': [
            'Travel insurance',
            'Medical insurance',
            'Education insurance',
          ...
          'correctAnswer': 1, // B - Medical insurance
        },
        {
          'question': 'What does Life/TPD insurance provide?',
          'options': [
            'Pays for car repairs',
            'Covers hospital food',
            'Cash payout if you die or become permanently disabled',
            'Covers house damage'
          ],
          'correctAnswer': 2, // C - Cash payout if you die or become permanently disabled
        },
        {
          'question': 'Which type of plan is suitable for people with a tight budget?',
          'options': [
            'Investment-linked plan',
            'Standalone or term plan',
            'Property plan',
            'Travel plan'
          ],
          'correctAnswer': 1, // B - Standalone or term plan
        },
      ];
    } else {
      // Types of Insurance Company in Malaysia questions
      return [
        {
          'question': 'Which type of insurance company mainly provides protection for people\'s lives and health?',
          'options': [
          ...
          'correctAnswer': 0, // A - Life insurance company
        },
        {
          'question': 'Which type of insurance covers assets like cars, houses, and property?',
          'options': [
            'Life insurance',
            'General insurance',
            'Travel agency',
          ...
          'correctAnswer': 1, // B - General insurance
        },
        {
          'question': 'Which organization regulates insurance companies in Malaysia?',
          'options': [
          ...
          'correctAnswer': 1, // B - Bank Negara Malaysia
        },
      ];
    }
  }
                                      ...
                                      Row(
                                        children: [
                                          Container(
                                            ...
                                            child: Text(
                                              _selectedTopic == 'Essence of Insurance Selection' ? '📋' : '🏢',
