// features/insurance/goals/policy_types.dart
...
class PolicyTypesPage extends StatefulWidget {
  const PolicyTypesPage({super.key});

  @override
  State<PolicyTypesPage> createState() => _PolicyTypesPageState();
}

class _PolicyTypesPageState extends State<PolicyTypesPage> {
  // Dropdown selection
  ...
  // Video players
  late VideoPlayerController _traditionalController;
  late VideoPlayerController _investmentController;
  late Future<void> _traditionalInitializeFuture;
  late Future<void> _investmentInitializeFuture;
  ...
  void initState() {
    ...
    // Initialize traditional video controller (Traditional Life Insurance Policy)
    _traditionalController = VideoPlayerController.asset('assets/video/traditional.mp4');
    _traditionalInitializeFuture = _traditionalController.initialize().then((_) {
      _traditionalController.setLooping(false);
      _traditionalController.setVolume(1.0);
      if (mounted) setState(() {});
    }).catchError((error) {
      debugPrint('Error loading traditional video: $error');
    });

    // Initialize investment video controller (Investment-linked Insurance Policy)
    _investmentController = VideoPlayerController.asset('assets/video/investment.mp4');
    _investmentInitializeFuture = _investmentController.initialize().then((_) {
      _investmentController.setLooping(false);
      _investmentController.setVolume(1.0);
      if (mounted) setState(() {});
    }).catchError((error) {
      debugPrint('Error loading investment video: $error');
    });
  }
  ...
  void _onTopicChanged(String? newValue) {
    setState(() {
      ...
      if (newValue == 'Traditional Life Insurance Policy') {
        _activeController = _traditionalController;
        _traditionalInitializeFuture.then((_) {
          if (mounted) {
            setState(() {
      ...
      } else if (newValue == 'Investment-linked Insurance Policy') {
        _activeController = _investmentController;
        _investmentInitializeFuture.then((_) {
          if (mounted) {
            setState(() {
              _isVideoInitialized = true;
            });
  ...
  List<Map<String, dynamic>> _getQuestionsForTopic() {
    if (_selectedTopic == 'Traditional Life Insurance Policy') {
      return [
        {
          'question': 'What is a traditional insurance policy?',
          'options': [
            'Insurance that only provides protection coverage',
            'Insurance that invests heavily in stocks',
            'Insurance for buying cars',
            'Insurance for travel only'
          ],
          'correctAnswer': 0, // A - Insurance that only provides protection coverage
        },
        {
          'question': 'Which of the following is a feature of a traditional insurance policy?',
          'options': [
            'High investment risk',
            'Simple and stable coverage',
            'Only for businesses',
            'No premium payment'
          ],
          'correctAnswer': 1, // B - Simple and stable coverage
        },
        {
          'question': 'Why do some people choose traditional policies?',
          'options': [
            'For stable protection and lower risk',
            'To become rich quickly',
            'To avoid paying premiums',
            'To buy houses'
          ],
          'correctAnswer': 0, // A - For stable protection and lower risk
        },
      ];
    } else {
      // Investment-linked Insurance Policy questions
      return [
        {
          'question': 'What is an investment-linked policy?',
          'options': [
            'Insurance combined with investment',
            'Insurance for cars only',
            'Insurance for houses only',
            'Insurance without protection'
          ],
          'correctAnswer': 0, // A - Insurance combined with investment
        },
        {
          'question': 'What happens to part of the premium in an investment-linked policy?',
          'options': [
            'It is used to buy groceries',
            'It is invested in funds',
            'It is given to the government',
            'It is returned immediately'
          ],
          'correctAnswer': 1, // B - It is invested in funds
        },
        {
          'question': 'Why do people choose investment-linked policies?',
          'options': [
            'To combine protection and investment',
            'To avoid paying premiums',
            'To repair houses',
            'To pay school fees'
          ],
          'correctAnswer': 0, // A - To combine protection and investment
        },
      ];
    }
  }
  ...
  String _getTopicEmoji() {
    if (_selectedTopic == 'Traditional Life Insurance Policy') {
      return '📜';
    } else if (_selectedTopic == 'Investment-linked Insurance Policy') {
      return '📈💰';
    }
    return '📋';
  }
